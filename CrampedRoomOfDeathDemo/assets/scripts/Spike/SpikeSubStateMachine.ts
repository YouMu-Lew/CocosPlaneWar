...
export default class SpikeSubStateMachine extends SubStateMachine {
	constructor(fsm: StateMachine, url: string, wrapMode: AnimationClip.WrapMode = AnimationClip.WrapMode.Normal) {
		...
		this.stateMachines.set(SPIKES_COUNT_ENUM.THREE, new State(fsm, `${this.baseURL}/three`, this.wrapMode));
		this.stateMachines.set(SPIKES_COUNT_ENUM.FOUR, new State(fsm, `${this.baseURL}/four`, this.wrapMode));
		this.stateMachines.set(SPIKES_COUNT_ENUM.FIVE, new State(fsm, `${this.baseURL}/five`, this.wrapMode));
	}
